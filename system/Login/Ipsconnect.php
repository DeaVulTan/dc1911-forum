...
class _Ipsconnect extends LoginAbstract
{
	...
	{
		$result = parent::get__enabled();

		/* If we are not enabled, determine if we are a master installation already - if so, we can't be enabled */
		if( $result === FALSE )
		{
			if( \IPS\Db::i()->select( 'COUNT(*)', 'core_ipsconnect_slaves' )->first() > 0 )
			{
				return NULL;
			}
		}
	...
	public function testSettings()
	{
		...
		/* If we are already a master, we can't be a slave too */
		if( \IPS\Db::i()->select( 'COUNT(*)', 'core_ipsconnect_slaves' )->first() > 0 )
		{
			throw new \InvalidArgumentException( \IPS\Member::loggedIn()->language()->addToStack('login_error_ipsconnect_master') );
		}
