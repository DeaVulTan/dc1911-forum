...
class _Db extends \mysqli
{
	...
	public static function i( $identifier=NULL, $connectionSettings=array() )
	{
		...
		if( !isset( static::$multitons[ $identifier ] ) )
		{
			...
			static::$multitons[ $identifier ] = @new $classname(
			...
			{
				throw new \IPS\Db\Exception( $error, static::$multitons[ $identifier ]->connect_errno );
			}
			...
			if( !static::$multitons[ $identifier ]->set_charset( static::$multitons[ $identifier ]->charset ) )
			{
				...
				static::$multitons[ $identifier ]->set_charset( static::$multitons[ $identifier ]->charset );
			}

			if ( \IPS\IN_DEV )
			{
				static::$multitons[ $identifier ]->query( "SET sql_mode='STRICT_ALL_TABLES,ONLY_FULL_GROUP_BY'" );
			}
	...
	public function preparedQuery( $query, array $_binds )
	{
		...
		{	
			/* Log */
			$this->log( static::_replaceBinds( $query, $binds ) );	
		}
		...
		{
			$this->returnQuery = FALSE;
			return static::_replaceBinds( $query, $binds );
		}
	 ...
	 * @param	string		$table			Table name
	 * @param	array		$definition		New index definition (see \IPS\Db::compileIndexDefinition for details)
	 * @param	array|null	$data			New table definition, or NULL to fetch current definition
	 * @return	void
	 */
	public function buildIndex( $table, $definition, $data=NULL )
	{
		$indexName	= $definition['name'];
		$data		= $data ?: $this->getTableDefinition( $table, FALSE, TRUE );
