...
EOF;
	...
	public function databaseCheck()
	{
		...
		foreach( json_decode( file_get_contents( \IPS\ROOT_PATH . "/applications/{$this->directory}/data/schema.json" ), TRUE ) as $tableName => $tableDefinition )
		{
			...
			try
			{
				$origLocalDefinition	= \IPS\Db::i()->getTableDefinition( $tableName );
				$localDefinition		= \IPS\Db::i()->normalizeDefinition( $origLocalDefinition );
							...
							foreach( $localDefinition['indexes'] as $indexName => $indexData )
							{
								...
								foreach( $indexData['columns'] as $indexColumn )
								{
									...
									if( $indexColumn == $columnName AND !in_array( $indexName, $dropped ) )
									{
										...
										/* We only want to compare lengths, because the regular index checking for indexes would already catch if
											the index is missing a column, etc. We need to check $origLocalDefinition because $localDefinition
											will wipe out the length to NULL, so the check of the index fails in that case */
										elseif( $thisIndex['indexes'][ $indexName ]['length'] !== $origLocalDefinition['indexes'][ $indexName ]['length'] )
										{
											$tableChanges[] = "DROP INDEX `{$db->escape_string( $indexName )}`";
