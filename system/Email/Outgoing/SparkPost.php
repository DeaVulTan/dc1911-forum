...
class _SparkPost extends \IPS\Email
{
	...
	{	
		...
		/* Set SparkPost\s special header which sets options like whether or not to use click tracking */
		$sparkPostHeaderData = array(
			'cc'		=> $cc,
			'bcc'		=> $bcc,
			'options'	=> array(
		...
		{
			$sparkPostHeaderData['options']['ip_pool'] = \IPS\Settings::i()->sparkpost_ip_pool;
		}
		$additionalHeaders['X-MSYS-API'] = json_encode( $sparkPostHeaderData );
		...
		$smtp->_sendCompiled( $fromEmail, $recipieintsForSmtp, $this->compileFullEmail( $to, $cc, array(), $fromEmail, $fromName, $additionalHeaders ) );
	}
