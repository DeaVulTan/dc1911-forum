...
abstract class _Widget extends \IPS\Widget\PermissionCache
{
	...
	protected function buildWhere()
	{
		...
		/* Don't show items with no comments */
		if ( $class::$firstCommentRequired AND isset( $class::$databaseColumnMap['first_comment_id'] ) )
		{
			$where[] = array( $class::$databaseTable . "." . $class::$databaseColumnMap['first_comment_id'] . " <> 0" );
		}
	...
	public function render()
	{
		...
		if ( isset( $this->configuration['widget_feed_tags'] ) and is_array( $this->configuration['widget_feed_tags'] ) and count( $this->configuration['widget_feed_tags'] ) )
		{
			...
			$where [] = array( 'primary_id_field IN (?)', \IPS\Db::i()->select( 'tag_meta_id', 'core_tags', array( array_merge( array( 'tag_meta_app=? AND tag_meta_area=? AND (' . implode( ' OR ', $tagWhere ) . ')' ), $binds ) ) ) );
		}
