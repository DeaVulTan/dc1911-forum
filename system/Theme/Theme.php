...
class _Theme extends \IPS\Node\Model
{
	...
	public function getCustomSettingField( $row )
	{
		...
		{					
			...
			$suffix = ( $row['sc_type'] == 'Color' and isset( $row['sv_value'] ) and ( $row['sc_default'] != $row['sv_value'] ) ) ? \IPS\Theme::i()->getTemplate( 'customization', 'core', 'admin' )->themeSettingRevert( $this->id, $row ) : NULL;
