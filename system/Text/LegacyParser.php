...
{	
	...
	public function parse( $value )
	{
		...
		foreach( $matches[0] as $k => $m )
		{
			...
			if( ! mb_strpos( $matches[1][ $k ], \IPS\Settings::i()->base_url ) AND ( mb_strpos( $matches[0][ $k ], 'youtube.co' ) OR mb_strpos( $matches[0][ $k ], 'youtu.be' ) ) and ! mb_strpos( $matches[0][ $k ], 'nomediaparse' ) AND $response = \IPS\Text\Parser::embeddableMedia( $matches[1][ $k ] ) )
			{
				$urls[ $c ] = $response;
			}
