...
class _Awards extends \IPS\Node\Model implements \IPS\Node\Permissions
{
    ...
    public function form( &$form )
    {
        ...
        //$form->add( new \IPS\Helpers\Form\YesNo( 'award_restrictions', isset( $this->restrictions ) ? $this->restrictions : 1, FALSE, array() ) );
        //$form->add( new \IPS\Helpers\Form\YesNo( 'award_self', isset( $this->self ) ? $this->self : 1, FALSE, array() ) );
    ...
    public function getButtons( $url, $subnode = FALSE )
    {
        ...
        $buttons['awarded_manage_members'] = array(
            ...
            'link'  => \IPS\Http\Url::internal( "app=awards&module=awards&controller=manage&do=manage_members&subnode=1" . "&id=" . $this->id ),
        );
    ...
    public function awardTo( \IPS\Member $member = NULL, $checkPerms = TRUE, \IPS\Member $giver = NULL, $reason = NULL )
    {
        ...
        /*if( $checkPerms and $this->obtainable_enabled AND $this->obtainable )
        {
            throw new \Exception( 'Sorry to be able to award you must disable the limited edition still available option.' );
        }

        if ( $checkPerms and !$this->can( 'self', $member ) AND $giver->member_id == $member->member_id )
        {
            throw new \Exception( 'You can not award yourself.' );
        }*/
