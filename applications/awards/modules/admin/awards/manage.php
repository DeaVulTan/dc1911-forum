...
class _manage extends \IPS\Node\Controller
{
     ...
     */
    protected function manage_members()
    {
        ...
        $url = \IPS\Http\Url::internal( 'app=awards&module=awards&controller=manage&do=manage_members&subnode=1&id=' . $item->id );
        ...
        $table->rootButtons = array(
            'add' => array(
                'icon'  => 'plus',
                'title' => 'award_member',
                'link'  => \IPS\Http\Url::internal( 'app=awards&module=awards&controller=manage&do=add_award&subnode=1' . '&id=' . $item->id ),
    ...
    protected function add_award()
    {
        ...
        {
            \IPS\Output::i()->error( 'credits_obtainable_err', 'IA457/E', 404, '' );
        }
        ...
        if ( $values = $form->values() )
        {
            ...
            {
                throw new \Exception( 'awards_self_err' );
            }

            if( $award->obtainable_enabled AND $award->obtainable )
            {
                throw new \Exception( 'Sorry to be able to award you must disable the limited edition still available option.' );
            }
            ...
            \IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=awards&module=awards&controller=manage&do=manage_members&subnode=1' . '&id=' . \IPS\Request::i()->id ), 'awarded_added' );
    ...
    protected function edit()
    {
        ...
        if ( $values = $form->values() )
        {
            ...
            \IPS\Output::i()->redirect( \IPS\Http\Url::internal( 'app=awards&module=awards&controller=manage&do=manage_members&subnode=1' . '&id=' . \IPS\Request::i()->award ), 'awarded_updated' );
