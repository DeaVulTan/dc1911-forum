...
class _Purchase extends \IPS\Node\Model
{
	...
	public function canRenewUntil( \IPS\Member $member = NULL, $inCycles = FALSE, $admin = FALSE )
	{
		...
		if ( !$admin and $this->cancelled or !$this->expire or !$this->renewals or $this->billing_agreement )
		{
			return FALSE;
		}
		try
	...
	public function canCancel( \IPS\Member $member = NULL )
	{
		...
		return !$this->grouped_renewals and !$this->billing_agreement and $this->renewals and $this->active;
	}
	...
	public function buttons( $ref='v' )
	{
		...
		if ( !$this->grouped_renewals and !$this->billing_agreement and $this->renewals and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'payments', 'invoices_add' ) )
		{
			$return['renew'] = array(
				'icon'	=> 'refresh',
		...
		if ( !$this->billing_agreement and $this->parent() and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'customers', 'purchases_edit' ) )
		{
		...
		if( !$this->billing_agreement and !$this->grouped_renewals and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'customers', 'purchases_transfer' ) )
		{
		...
		if ( !$this->billing_agreement and !$this->grouped_renewals and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'customers', 'purchases_cancel' ) )
		{
			if ( $this->cancelled )
			{
		...
		if ( !$this->billing_agreement and !$this->grouped_renewals and \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'customers', 'purchases_delete' ) )
		{
			$return['delete'] = array(
				'icon'	=> 'times-circle',
				'title'	=> 'delete',
