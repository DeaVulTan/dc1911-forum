...
class _Invoice extends \IPS\Patterns\ActiveRecord
{
	...
	public function summary()
	{
			...
			if ( $item instanceof \IPS\nexus\extensions\nexus\Item\CouponDiscount )
			{
				if ( isset( $item->extra['type'] ) )
				{
					...
					else
					{
						...
						{
							if ( $_item != $item and !( $item instanceof \IPS\nexus\extensions\nexus\Item\ShippingCharge ) )
							{
								$itemCount++;
							}
						}
					...
					foreach ( $this->items as $k => $_item )
					{
						if ( $_item != $item and !( $item instanceof \IPS\nexus\extensions\nexus\Item\ShippingCharge ) and ( !isset( $item->extra['items'] ) or in_array( $k, $item->extra['items'] ) ) )
						{
							...
							{
								$couponTaxReductions[ $k ] = $_item->linePrice()->amount->multiply( new \IPS\Math\Number("{$item->extra['value']}") )->divide( new \IPS\Math\Number('100') );
							}
							else
							{
								$couponTaxReductions[ $k ] = ( new \IPS\Math\Number( "{$item->extra['value']}" ) )->divide( new \IPS\Math\Number( "{$itemCount}" ) )->multiply( new \IPS\Math\Number( '-1' ) );
							}
						}
					}
				}
		...
		foreach ( $taxableAmounts as $taxId => $amount )
		{
			...
			$amount = $amount->multiply( new \IPS\Math\Number( number_format( $rate, 2, '.', '' ) ) );
