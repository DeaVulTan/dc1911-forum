...
class _Categories
{
	...
	public function attachmentPermissionCheck( $member, $id1, $id2, $id3 )
	{
		try
		{
			$cat = \IPS\videos\Category::load( $id1 );
	...
	{
		return \IPS\videos\Category::load( $id1 );
	}
	...
	protected function performRebuild( $offset, $max, $callback )
	{
		...
		/* Language bits */
		foreach( \IPS\Db::i()->select( '*', 'core_sys_lang_words', "word_key LIKE 'videos_category_%_desc' OR word_key LIKE 'videos_category_%_rules'" ) as $word )
		{
			try
			{
	...
	{
		return \IPS\Db::i()->select( 'COUNT(*)', 'core_sys_lang_words', "word_key LIKE 'videos_category_%_desc' OR word_key LIKE 'videos_category_%_rules'" )->first();
	}
}