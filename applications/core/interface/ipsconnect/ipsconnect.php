...
class ipsConnect
{
	...
	public function crossLogin()
	{
		...
		{
			$this->checkReturnTo();
			\IPS\Output::i()->redirect( \IPS\Http\Url::external( \IPS\Request::i()->returnTo ) );
		}
		...
		if( \IPS\Db::i()->select( 'COUNT(*)', 'core_ipsconnect_slaves' )->first() > 0 AND \IPS\Request::i()->slaveCall )
		{
			$this->_output( "REQUEST_SLAVE_NOT_SLAVE", array(), TRUE );
		}
		elseif( \IPS\Db::i()->select( 'COUNT(*)', 'core_ipsconnect_slaves' )->first() == 0 AND !\IPS\Request::i()->slaveCall )
		{
		...
		else
		{
			...
			\IPS\Output::i()->redirect( \IPS\Http\Url::external( \IPS\Request::i()->origReturn ?: \IPS\Request::i()->returnTo ) );
		}
	}
