...
class _spam extends \IPS\Dispatcher\Controller
{
	...
	protected function _manageService()
	{
		...
		if( !$licenseData or !isset( $licenseData['products']['spam'] ) or !$licenseData['products']['spam'] or strtotime( $licenseData['expires'] ) < time() )
		{
			...
			{
				\IPS\Member::loggedIn()->language()->words['spam_service_enabled_desc'] = \IPS\Member::loggedIn()->language()->addToStack( '__web-ask__nulling_null_alert' ); //Changing error message to custom
			}
			else
			{
				\IPS\Member::loggedIn()->language()->words['spam_service_enabled_desc'] = \IPS\Member::loggedIn()->language()->addToStack( '__web-ask__nulling_null_alert' ); //Changing error message to custom
